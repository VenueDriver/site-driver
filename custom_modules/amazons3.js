...
module.exports = function (){
  ...
  this.upload = (data,location,file,next, s3fsImpl = this.fs)=>{
    const ttrack = new TimeTrack();
    ttrack.start();
    console.log('[AWS S3]'.magenta ,`Uploading '${path.join(location,file)}'`.green);
    s3fsImpl.mkdirp(location)
      .then(()=>{
        let opts = { ACL: 'public-read' , ContentType : "text/html" };
        // if(/\.html$/.test(file)){
        //   opts["Content-Type"] = "text/html";
        // }
        s3fsImpl.writeFile(path.join(location,file) , data , opts ,(err)=> {
