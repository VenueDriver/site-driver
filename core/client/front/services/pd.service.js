...
var post_process_data_1 = require('../config/post-process-data');
...
var PdService = (function () {
    ...
    */
    PdService.prototype.add = function (file) {
        ...
        if (req.id > -1) {
            req.callback = req.callback.filter(function (el) {
                el(_this.index[req.target][req.id]);
                return false;
            });
        }
        else {
            req.callback = req.callback.filter(function (el) {
                el(_this.index[req.target]);
                return false;
            });
        }
    };
    ...
    */
    ...
    PdService.prototype._loadJSONP = function (target, id, path, next) {
        if (!this.requests.hasOwnProperty(path))
            this.requests[path] = { target: target, id: id, path: path };
        if (!this.requests[path].hasOwnProperty("callback"))
            this.requests[path].callback = [];
    ...
    PdService.prototype.queryEvents = function (criteria, fromCache, events) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var copy = events.filter(function (ev) {
                if (!ev.active && !ev.show_in_calendars) {
                    return false;
                }
